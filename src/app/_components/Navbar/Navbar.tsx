                    ...
                    <ul className="font-medium flex flex-col p-4 md:p-0 rounded-base bg-neutral-secondary-soft md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-neutral-primary">
                        {path.map((elem) => {
                            return <li key={elem.content}>
                                <Link href={elem.href} className={`block py-2 px-3 text-heading rounded font-bold ${pathname === elem.href ? 'active' : ''} hover:bg-neutral-tertiary md:hover:bg-transparent md:border-0 md:hover:text-fg-brand md:p- md:dark:hover:bg-transparent`}>{elem.content}</Link>
                            </li>
                        })}
                    </ul>
